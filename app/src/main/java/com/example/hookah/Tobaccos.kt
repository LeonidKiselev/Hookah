...
class Tobaccos : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.tobaccos)
        ...
        listView.setOnItemClickListener { _, _, position, _ ->
            ...
            val intent = Intent(this, ViewTobacco::class.java)
            intent.putExtra("name", item)
            startActivity(intent)
        }
    }
    fun onClickAddTobacco(view: View) {
        if ((this.getSharedPreferences("tobacco names", MODE_PRIVATE).all as Map<*, *>).count() == 8) {
            Toast.makeText(this, "maximum count of tobaccos reached", Toast.LENGTH_SHORT).show()
        } else {
            val intent = Intent(this, AddTobacco::class.java)
            startActivity(intent)
        }
    }
}